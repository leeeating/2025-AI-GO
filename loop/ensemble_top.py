...
res = {}
for idx in range(15):
    selected_features = gain.iloc[:, idx].nlargest(n=500).index    
    ...
    X_test_known = X_test_pt[zero_np]
    y_test_known = np.zeros(X_test_known.shape[0])

    X_train_aug = np.concatenate([X_train_pt, X_test_known], axis=0)
    ...
    joblib.dump(save_dict, f'./ense_result/add_zero/trail_{idx}.pkl')
